...
交互式 skill 现在要求这些用户级 Codex session hooks 存在；如果缺失，会在初次扫描仓库后、第一次澄清提问前自动补齐。你也可以手动预装或查看状态：
...
这些 hooks 只会附着到后续那些明确看起来像 `codex-autoresearch` 工作的 Codex 会话上。它们不会追溯性改变你眼前这个已经打开的前台会话，也不会误伤同一仓库里的无关 Codex 对话。

- 如果这是 skill 刚刚在当前会话里自动安装的，`background` 可以立刻受益。
- 当前已经打开的前台会话不会在中途突然吃到 hooks。
- 托管 `background` run 会把自己配置过的 artifact 路径显式传给这些嵌套会话，所以自定义 `--results-path` / `--state-path` 在后台模式下仍然有效。
- 当前这个 `foreground` 会话不会在中途突然吃到 hooks；如果你希望前台也用上它，需要在**新的 Codex 会话**里重新打开/恢复当前线程再继续。CLI 里这通常是 `codex resume`；桌面端则是在新会话里重新打开同一个线程。
