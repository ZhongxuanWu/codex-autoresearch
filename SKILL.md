...
- For clearly long-running foreground/background runs, check `python3 <skill-root>/scripts/autoresearch_hooks_ctl.py status` before the final mode choice and then follow the hooks decision flow in `references/interaction-wizard.md`. Keep the user-facing note short. Never auto-install.
